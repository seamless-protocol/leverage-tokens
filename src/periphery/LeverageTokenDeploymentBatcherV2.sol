...
import {ILeverageTokenDeploymentBatcherV2} from "../interfaces/periphery/ILeverageTokenDeploymentBatcherV2.sol";
...
import {IAaveLendingAdapter} from "../interfaces/IAaveLendingAdapter.sol";
import {IAaveLendingAdapterFactory} from "../interfaces/IAaveLendingAdapterFactory.sol";
...
/**
 * @title LeverageTokenDeploymentBatcherV2
 * @dev The LeverageTokenDeploymentBatcherV2 is a periphery contract that can be used to batch deployment of a LeverageToken
 ...
 */
contract LeverageTokenDeploymentBatcherV2 is ILeverageTokenDeploymentBatcherV2 {
    /// @inheritdoc ILeverageTokenDeploymentBatcherV2
    ...
    /// @inheritdoc ILeverageTokenDeploymentBatcherV2
    IAaveLendingAdapterFactory public immutable aaveLendingAdapterFactory;

    /// @notice Constructor
    /// @param _leverageManager The LeverageManager contract
    /// @param _morphoLendingAdapterFactory The MorphoLendingAdapterFactory contract
    /// @param _aaveLendingAdapterFactory The AaveLendingAdapterFactory contract
    constructor(
        ILeverageManager _leverageManager,
        IMorphoLendingAdapterFactory _morphoLendingAdapterFactory,
        IAaveLendingAdapterFactory _aaveLendingAdapterFactory
    ) {
        leverageManager = _leverageManager;
        morphoLendingAdapterFactory = _morphoLendingAdapterFactory;
        aaveLendingAdapterFactory = _aaveLendingAdapterFactory;
    }

    /// @inheritdoc ILeverageTokenDeploymentBatcherV2
    function deployLeverageTokenAndDepositWithMorpho(
        ...
        return _deployLeverageTokenAndDeposit(
    ...
    /// @inheritdoc ILeverageTokenDeploymentBatcherV2
    function deployLeverageTokenAndDepositWithAave(
        LeverageTokenDeploymentParams memory leverageTokenDeploymentParams,
        AaveLendingAdapterDeploymentParams memory lendingAdapterDeploymentParams,
        RebalanceAdapterDeploymentParams memory rebalanceAdapterDeploymentParams,
        uint256 collateral,
        uint256 minShares
    ) public returns (ILeverageToken, ActionData memory) {
        IAaveLendingAdapter lendingAdapter = aaveLendingAdapterFactory.deployAdapter(
            lendingAdapterDeploymentParams.collateralAsset,
            lendingAdapterDeploymentParams.debtAsset,
            address(this),
            salt(msg.sender, lendingAdapterDeploymentParams.baseSalt)
        );
    ...
    /// @notice Internal function to deploy a leverage token with a given lending adapter and deposit collateral
    /// @param leverageTokenDeploymentParams The parameters for the leverage token deployment
    /// @param lendingAdapter The lending adapter to use for the leverage token
    ...
    function _deployLeverageTokenAndDeposit(
        LeverageTokenDeploymentParams memory leverageTokenDeploymentParams,
        ILendingAdapter lendingAdapter,
        RebalanceAdapterDeploymentParams memory rebalanceAdapterDeploymentParams,
        uint256 collateral,
        uint256 minShares
    ) internal returns (ILeverageToken, ActionData memory) {
