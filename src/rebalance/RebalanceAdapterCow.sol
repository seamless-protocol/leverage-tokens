...
import {DutchAuctionRebalanceAdapterCow} from "src/rebalance/DutchAuctionRebalanceAdapterCow.sol";
 ...
 */
contract RebalanceAdapterCow is
    IRebalanceAdapter,
    UUPSUpgradeable,
    OwnableUpgradeable,
    CollateralRatiosRebalanceAdapter,
    DutchAuctionRebalanceAdapterCow,
    PreLiquidationRebalanceAdapter
{
    ...
    struct RebalanceAdapterInitParams {
        ...
        /// @notice The CowTrampoline Address (testing only allows take from this address)
        address cowTrampoline;
    }
    ...
    function initialize(RebalanceAdapterInitParams memory params) external initializer {
        ...
        cowTrampoline = params.cowTrampoline;
    ...
    function getLeverageTokenTargetCollateralRatio()
        public
        view
        override(DutchAuctionRebalanceAdapterCow, CollateralRatiosRebalanceAdapter)
    ...
    function isEligibleForRebalance(ILeverageToken token, LeverageTokenState memory state, address caller)
        ...
        returns (bool)
    {
        return ((DutchAuctionRebalanceAdapterCow.isEligibleForRebalance(token, state, caller)
    ...
    function isStateAfterRebalanceValid(ILeverageToken token, LeverageTokenState memory stateBefore)
        ...
        returns (bool)
    {
        return (CollateralRatiosRebalanceAdapter.isStateAfterRebalanceValid(token, stateBefore)
