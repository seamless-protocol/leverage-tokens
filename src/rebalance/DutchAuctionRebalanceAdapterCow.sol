 ...
 */
abstract contract DutchAuctionRebalanceAdapterCow is IDutchAuctionRebalanceAdapter, Initializable {
    // Test Cow solving by only allowing take auctions by cowTrampoline
    address public cowTrampoline;
    /// @notice Error thrown when the caller is unauthorized to call a function

    error SenderNotCowSolver();
    ...
    modifier onlyCowTrampoline() {
        if (msg.sender != address(cowTrampoline)) revert SenderNotCowSolver();
        _;
    }
    ...
    function createAuction() external {
        ...
        Auction memory auction = Auction({
            isOverCollateralized: isOverCollateralized, startTimestamp: startTimestamp, endTimestamp: endTimestamp
        });
    ...
    /// @inheritdoc IDutchAuctionRebalanceAdapter
    function take(uint256 amountOut) external onlyCowTrampoline {
        if (!isAuctionValid()) {
