...
import {IAaveLendingAdapter} from "../IAaveLendingAdapter.sol";
import {IAaveLendingAdapterFactory} from "../IAaveLendingAdapterFactory.sol";
import {IRebalanceAdapter} from "../IRebalanceAdapter.sol";
import {ActionData} from "../../types/DataTypes.sol";

/// @title ILeverageTokenDeploymentBatcherV2
/// @notice Interface for LeverageTokenDeploymentBatcherV2 - batches deployment and initial deposit for LeverageTokens
interface ILeverageTokenDeploymentBatcherV2 {
    ...
    struct AaveLendingAdapterDeploymentParams {
    ...
    /// @notice The AaveLendingAdapterFactory contract
    function aaveLendingAdapterFactory() external view returns (IAaveLendingAdapterFactory);

    /// @notice Deploys a LeverageToken with a Morpho lending adapter and deposits collateral into it.
    /// The sender receives the shares and debt from the deposit.
    /// @param leverageTokenDeploymentParams The parameters for the leverage token deployment
    /// @param lendingAdapterDeploymentParams The parameters for the Morpho lending adapter deployment
    ...
    /// @notice Deploys a LeverageToken with an Aave lending adapter and deposits collateral into it.
    /// The sender receives the shares and debt from the deposit.
    /// @param leverageTokenDeploymentParams The parameters for the leverage token deployment
    /// @param lendingAdapterDeploymentParams The parameters for the Aave lending adapter deployment
    /// @param rebalanceAdapterDeploymentParams The parameters for the rebalance adapter deployment
    /// @param collateral The collateral to deposit into the leverage token
    /// @param minShares The minimum number of shares to receive from the deposit
    /// @return The leverage token and the action data for the deposit
    /// @dev The lending adapter deployed is an `AaveLendingAdapter`. The `aaveLendingAdapterFactory` is used to deploy it
    /// as a ERC-1167 minimal proxy.
    /// @dev The rebalance adapter is deployed from the `rebalanceAdapterDeploymentParams.implementation` address as a
    /// UUPS proxy.
