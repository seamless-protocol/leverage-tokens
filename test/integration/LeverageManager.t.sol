...
contract LeverageManagerTest is MorphoLendingAdapterTest {
    ...
    function testFork_deposit_PriceChangedBetweenDeposits() public {
        uint256 equityInCollateralAsset = 10 ether;
        uint256 collateralToAdd = 2 * equityInCollateralAsset;
        uint256 debtToBorrow = 23762_369619; // 23,762.369619
        ...
        vm.mockCall(
            address(oracle), abi.encodeWithSelector(IOracle.price.selector), abi.encode(IOracle(oracle).price() * 2)
        );

        equityInCollateralAsset = 10 ether;

        (uint256 collateral, uint256 debt,,) = leverageManager.previewDeposit(strategy, equityInCollateralAsset);

        deal(address(WETH), user, collateral);
        StrategyState memory stateBefore = LeverageManager(address(leverageManager))._getStrategyState(strategy);

        vm.startPrank(user);
        WETH.approve(address(leverageManager), collateral);
        leverageManager.deposit(strategy, equityInCollateralAsset, 0);
        vm.stopPrank();

        assertEq(WETH.balanceOf(user), 0);

        StrategyState memory stateAfter = LeverageManager(address(leverageManager))._getStrategyState(strategy);
