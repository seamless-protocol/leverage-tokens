...
contract DeployLeverageTokenAndDepositTest is LeverageTokenDeploymentBatcherTest {
    function testFork_deployLeverageTokenAndDeposit() public {
        ...
        MorphoLendingAdapter lendingAdapter =
            MorphoLendingAdapter(address(leverageManager.getLeverageTokenLendingAdapter(_leverageToken)));
        assertEq(address(lendingAdapter.leverageManager()), address(leverageManager));
        assertEq(abi.encode(lendingAdapter.morphoMarketId()), abi.encode(lendingAdapterDeploymentParams.morphoMarketId));
        assertEq(lendingAdapter.isUsed(), true);

        // Sanity check: RebalanceAdapter
        RebalanceAdapter _rebalanceAdapter =
            RebalanceAdapter(address(leverageManager.getLeverageTokenRebalanceAdapter(_leverageToken)));
        assertEq(address(_rebalanceAdapter.getLeverageManager()), address(leverageManager));
        assertEq(
            _rebalanceAdapter.getLeverageTokenTargetCollateralRatio(),
            rebalanceAdapterDeploymentParams.targetCollateralRatio
        );
        assertEq(_rebalanceAdapter.owner(), owner);
