...
contract SetManagementFeeTest is FeeManagerTest {
    ...
    function testFuzz_setManagementFee(ILeverageToken token, uint16 managementFee) public {
        managementFee = uint16(bound(managementFee, 0, MAX_MANAGEMENT_FEE));
    ...
    /// forge-config: default.fuzz.runs = 1
    function testFuzz_setManagementFee_RevertIf_FeeTooHigh(ILeverageToken token, uint16 managementFee) public {
        managementFee = uint16(bound(managementFee, MAX_MANAGEMENT_FEE + 1, type(uint16).max));
