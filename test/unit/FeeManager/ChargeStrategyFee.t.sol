...
contract SetStrategyActionFeeTest is FeeManagerBaseTest {
    ...
    function test_chargeStrategyFee_FeeRoundedUp() public {
        ...
        for (uint256 i = 0; i < uint256(type(IFeeManager.Action).max) + 1; i++) {
            _setStrategyActionFee(feeManagerRole, strategyId, IFeeManager.Action(i), fee);

            uint256 amountAfterFee =
                feeManager.exposed_chargeStrategyFee(strategyId, amount, IFeeManager.Action.Deposit);

            assertEq(amountAfterFee, 0);
        }
    }

    function testFuzz_chargeStrategyFee(uint256 strategyId, uint256 amount, uint256 actionNum, uint256 fee) public {
        ...
        emit IFeeManager.FeeCharged(strategyId, action, amount, amount - expectedAmountAfterFee);

        uint256 amountAfterFee = feeManager.exposed_chargeStrategyFee(strategyId, amount, action);
