...
contract CalculateDebtAndSharesTest is LeverageManagerBaseTest {
    ...
    function test_calculateDebtAndShares() public {
        CalculateDebtAndSharesState memory state = CalculateDebtAndSharesState({
            ...
            targetRatio: 2 * BASE_RATIO, // 2x leverage also
            collateral: 1 ether,
        ...
        (uint256 debt, uint256 shares) = leverageManager.calculateDebtAndShares(
    ...
    function testFuzz_calculateDebtAndShares(CalculateDebtAndSharesState memory state) public {
        ...
        (uint256 debt, uint256 shares) =
            leverageManager.calculateDebtAndShares(strategy, leverageManager.getLendingContract(), collateral);

        uint256 expectedDebt = Math.mulDiv(convertedCollateral, BASE_RATIO, targetRatio, Math.Rounding.Ceil);
