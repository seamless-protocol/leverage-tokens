...
contract MintAndRepayMorphoFlashLoanTest is LeverageRouterMintBaseTest {
    function testFuzz_mintAndRepayMorphoFlashLoan(
        ...
        // Set the swap slippage to be the additional collateral required to repay the flash loan
    ...
    function testFuzz_mintAndRepayMorphoFlashLoan_RevertIf_MaxSwapCostExceeded(
        uint256 requiredCollateral,
        uint256 equityInCollateralAsset,
        uint256 swapAmountOut,
        ...
        // Mock the amount of collateral received from the debt swap to be less than required to repay the flash loan
        swapAmountOut = bound(swapAmountOut, 0, requiredFlashLoan - 1);
        leverageRouterMintBase.mock_setNextSwapAmountOut(swapAmountOut);

        // Set the swap slippage to be less than the additional collateral required to repay the flash loan
        maxSwapCostInCollateralAsset = bound(maxSwapCostInCollateralAsset, 0, requiredFlashLoan - swapAmountOut - 1);
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ILeverageRouterBase.MaxSwapCostExceeded.selector,
                requiredFlashLoan - swapAmountOut,
